...
#         optimizer_coco = torch.optim.Adam(params=model.mapping.parameters(), lr=self.lr, weight_decay=self.wd)
#         return optimizer_src, optimizer_tgt, optimizer_meta, optimizer_aug, optimizer_map, optimizer_coco
...
#             criterion, optimizer_src, optimizer_map, optimizer_meta, criterion_moco):
#         print('=====CDR Pretraining=====')
#         for i in range(self.epoch):
#             self.train(data_src, model, criterion, optimizer_src, i, stage='train_src')
#         print('==========MOCO==========')
#         for i in range(self.epoch):
#             self.train(data_meta, model, criterion_moco, optimizer_meta, i, stage='train_moco')
#         print('==========EMCDR==========')
#         for i in range(self.epoch):
...
#             print('MAE: {} RMSE: {}'.format(mae, rmse))
#         self.plot_scatter(model,data_test,stage='get_embedding')

#     def get_tgt_uids(self):
#         x_uid = ['uid']
#         data = pd.read_csv(self.tgt_path, header=None)
#         cols = ['uid', 'iid', 'y']
#         data.columns = cols
#         X = data[x_uid].values[0]
#         print(X)
#         return np.array(list(set(list(X))))

#     def plot_scatter(self,model,data_loader,stage):
#         print('plot')
#         model.eval()
#         device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # 若cuda可用则使用cuda
#         pred_emb=torch.empty((0,10)).to(device)
#         tgt_emb=torch.empty((0,10)).to(device)

#         with torch.no_grad():
#             for X,y in data_loader:
#                 device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # 若cuda可用则使用cuda
#                 model = model.to(device)  # 将模型移到适当的设备上
#                 X = X.to(device)  # 将数据移到适当的设备上
#                 pred,tgt=model(X,stage)
#                 pred_emb=torch.cat((pred_emb,pred))
#                 tgt_emb=torch.cat((tgt_emb,tgt))
#         # 使用 t-SNE 进行降维
#         pred_numpy=pred_emb.cpu().numpy()
#         tgt_numpy=tgt_emb.cpu().numpy()
#         tsne = TSNE(n_components=2, random_state=0)
#         embeddings_emcdr_2d = tsne.fit_transform(pred_numpy)
#         embeddings_ptupcdr_2d = tsne.fit_transform(tgt_numpy)

#         # 这里我们仅随机选取100个点进行可视化
#         indices = np.random.choice(len(embeddings_emcdr_2d), 100)

#         # 可视化
#         plt.figure(figsize=(12, 6))

#         plt.subplot(111)
#         plt.scatter(embeddings_emcdr_2d[indices, 0], embeddings_emcdr_2d[indices, 1], marker='*', label='preb',color='blue')
#         plt.scatter(embeddings_ptupcdr_2d[indices, 0], embeddings_ptupcdr_2d[indices, 1], marker='+', label='target',color='red')
#         plt.title('trained')

#         # plt.subplot(122)
#         # plt.scatter(embeddings_ptupcdr_2d[indices, 0], embeddings_ptupcdr_2d[indices, 1], label='PTUPCDR')
#         # plt.title('User embeddings from PTUPCDR')
...
#         optimizer_src, optimizer_tgt, optimizer_meta, optimizer_aug, optimizer_map, optimizer_coco = self.get_optimizer(model)
#         criterion = torch.nn.MSELoss()
#         criterion_moco = torch.nn.CrossEntropyLoss()

#         tgt_uids = self.get_tgt_uids()
#         model.set_tgt_uids(tgt_uids)
